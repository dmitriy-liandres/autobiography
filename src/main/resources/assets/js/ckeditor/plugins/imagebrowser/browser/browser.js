...
CkEditorImageBrowser.deletedImage = function (folderName, imageUrl, thumbUrl) {
    if (typeof(CkEditorImageBrowser.images[folderName]) === 'undefined') {
        CkEditorImageBrowser.folders.push(folderName);
        CkEditorImageBrowser.images[folderName] = [];
    }
...
CkEditorImageBrowser.initEventHandlers = function () {
    ...
    $(document).on('click', '.js-image-delete-link', function (event) {
        event.stopPropagation();
        var jParentElement = $(this).parent();
        $.ajax({
            url: jParentElement.data('url'),
            type: 'DELETE',
            dataType: 'text',
            success: function(result) {
                jParentElement.remove();
                var leftElements = $('.js-image-link');
                if(leftElements.length == 0){
                    window.opener.CKEDITOR.tools.callFunction(CkEditorImageBrowser.ckFunctionNum, "");
                    window.close();
                }
